...
import { Sparkles, Heart, CheckCircle2, Calendar, Flame, ChevronLeft, ChevronRight, Edit3 } from 'lucide-react';
...
export const DailyLogTab: React.FC<DailyLogTabProps> = ({
  ...
  const [painVas, setPainVas] = useState<number>(todayExisting?.painVas ?? 3);
  const [allodyniaLevel, setAllodyniaLevel] = useState<number>(todayExisting?.allodyniaLevel ?? 0);
  const [painLocations, setPainLocations] = useState<string[]>(
    todayExisting?.painLocations || []
  );
  const [exercises, setExercises] = useState(
    todayExisting?.exercises || {
      chairSquats: false,
      towelGather: false,
      husbandSoleCare: false,
      tensTherapy: false,
      calfStretch: false,
      walking: false,
    }
  ...
  // Switch form to a specific date (e.g. from calendar click)
  const handleSelectDateFromCalendar = (targetDateStr: string) => {
    const existing = existingLogs.find((l) => l.date === targetDateStr);
    setDate(targetDateStr);
    if (existing) {
      setCondition(existing.condition);
      setFatigueLevel(existing.fatigueLevel);
      setPainVas(existing.painVas);
      setAllodyniaLevel(existing.allodyniaLevel);
      setPainLocations(existing.painLocations || []);
      setExercises(existing.exercises);
      setPslDoseMg(existing.pslDoseMg);
      setBodyTemp(existing.bodyTemp);
      setStepCount(existing.stepCount);
      setWeather(existing.weather);
      setOshiEnergy(existing.oshiEnergy);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newLog: DailyLog = {
      id: existingForDate ? existingForDate.id : `log-${Date.now()}`,
      date,
      condition,
      fatigueLevel,
      painVas,
      ...
      oshiEnergy,
      memo,
      createdAt: existingForDate?.createdAt || new Date().toISOString(),
    };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      {/* Top Gamification Banner: Streak & Avatar Evolution */}
      <div className="glass-card rounded-3xl p-4 sm:p-5 border-2 border-pink-200 shadow-sm bg-gradient-to-r from-pink-50 via-purple-50 to-blue-50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3.5 text-left w-full sm:w-auto">
          <div className="relative cursor-pointer" onClick={onOpenEvolutionModal}>
            <div className="w-14 h-14 rounded-2xl p-1 bg-gradient-to-tr from-pink-400 via-purple-400 to-indigo-400 shadow-md">
              <div className="w-full h-full rounded-xl overflow-hidden bg-white">
                <img
                  src={
                    streakInfo.activeStyle === 'photo'
                      ? '/jeonghan_photo.jpg'
                      : streakInfo.activeStyle === 'illust'
                      ? '/jeonghan_avatar.jpg'
                      : '/jeonghan_dot.jpg'
                  }
                  alt="Jeonghan evolution"
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-black text-slate-800">
                連続 {streakInfo.currentStreak} 日達成！🔥
              </span>
              <span className="text-[10px] font-bold text-pink-600 bg-white px-2 py-0.2 rounded-full border border-pink-200">
                {streakInfo.badgeLabel}
              </span>
            </div>
            <p className="text-xs text-slate-600 mt-0.5">
              {streakInfo.cheerMessage}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white border border-pink-200 text-xs font-extrabold text-pink-600 shadow-xs hover:bg-pink-50 hover:scale-105 transition-all flex items-center gap-1.5"
          >
      ...
      {/* View Mode Toggle: [今日の入力 ✍️] vs [カレンダー記録 📅] */}
      <div className="flex items-center justify-between gap-3 bg-white/70 p-1.5 rounded-2xl border border-pink-100 shadow-xs">
        <button
          type="button"
        ...
        >
          ...
          <span>カレンダースタンプ記録 📅</span>
        </button>
      </div>

      {/* CALENDAR VIEW */}
      {viewMode === 'calendar' ? (
        <div className="glass-card rounded-3xl p-5 sm:p-6 border border-pink-100 shadow-sm text-left animate-fade-in">
          {/* Calendar Month Navigation */}
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-pink-100">
            <button
              onClick={prevMonth}
              className="p-2 rounded-xl bg-white border border-pink-200 text-slate-600 hover:bg-pink-50"
            >
          ...
          {/* Days of week */}
          <div className="grid grid-cols-7 gap-1.5 text-center text-xs font-extrabold text-slate-400 mb-2">
          ...
          <div className="grid grid-cols-7 gap-1.5 sm:gap-2">
            ...
            {Array.from({ length: firstDayOfWeek }).map((_, i) => (
              <div key={`blank-${i}`} className="min-h-16 sm:min-h-20 rounded-2xl bg-slate-50/40 opacity-40" />
            ))}
            ...
            {Array.from({ length: totalDaysInMonth }).map((_, i) => {
              ...
              const log = existingLogs.find((l) => l.date === dateStr);
              ...
              return (
                <button
                  ...
                  className={`min-h-16 sm:min-h-20 p-1.5 rounded-2xl border text-left transition-all relative flex flex-col justify-between ${
                    isToday
                      ? 'border-2 border-pink-400 bg-pink-50/60 shadow-xs'
                      : log
                      ? 'border-pink-200 bg-white/90 hover:border-pink-300 hover:scale-[1.02]'
                      : 'border-slate-100 bg-white/50 hover:bg-pink-50/20'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold ${isToday ? 'text-pink-600 font-black' : 'text-slate-700'}`}>
                      {dayNum}
                    </span>
                    {isToday && (
                      <span className="text-[9px] bg-pink-500 text-white px-1 rounded font-bold">
                        今日
                      </span>
                    )}
                  </div>
                  ...
                  {log ? (
                    <div className="flex flex-col items-center justify-center my-0.5">
                      <span className="text-xl sm:text-2xl animate-float">
                        {completedCount >= 2 ? '💎' : completedCount === 1 ? '👼' : '✨'}
                      </span>
                  ...
                  {log && (
                    <div className="text-[9px] text-slate-500 text-center font-bold bg-slate-100/80 rounded px-1">
                      VAS {log.painVas}
                    </div>
                  )}
                </button>
          ...
          <div className="mt-4 pt-3 border-t border-pink-100 flex flex-wrap items-center justify-between text-xs text-slate-500 gap-2">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1">💎 2種目以上達成</span>
              <span className="flex items-center gap-1">👼 1種目達成</span>
              <span className="flex items-center gap-1">✨ 記録済</span>
            </div>
            <span className="text-[11px] text-pink-600 font-bold">
              日付をタップするとその日の記録を確認・編集できます
            </span>
          </div>
        </div>
      ) : (
