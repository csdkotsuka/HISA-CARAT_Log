...
// Real-time listener for Daily Logs
...
// Real-time listener for PT Eval Docks
...
export const seedFirestoreIfEmpty = async (): Promise<boolean> => {
  try {
    ...
    if (ptSnap.empty) {
      console.log('Seeding initial PT docks to Firestore...');
    ...
    console.warn('Auto-seed to Firestore skipped (check security rules / permissions):', error);
    return false;
  }
};
